  ...
  try {
    ...
    if (typeof payload.maintenanceMode !== "boolean") {
      return jsonResponse(400, {
        error: "maintenanceMode must be provided"
      });
    }
    ...
    const { data, error } = await supabase.from("maintenance_state").upsert({
      id: 1,
      ...
      onConflict: "id"
    }).select("maintenance_mode, last_maintenance_at, supabase_status, analytics_status, app_version, last_aggregator_sync").single();
    if (error) {
      throw error;
    }
