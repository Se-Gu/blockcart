...
serve(async (req)=>{
  ...
  try {
    if (!receipt_id || !image_url) throw new Error("Missing receipt_id or image_url");
    const match = image_url.match(/receipts\/(.+)$/);
    if (!match) throw new Error("Invalid image URL");
    const objectPath = match[1];
    // 🔐 Signed URL
    const { data: signed, error: signErr } = await supabase.storage.from("receipts").createSignedUrl(objectPath, 120);
    ...
    const llmRes = await fetch("https://api.openai.com/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: "system",
            content: "You are a receipt parser. Return only valid JSON with keys: " + "store (string), location (string or null), date (ISO string), time (string or null), " + "payment_method (string or null), total (number), " + "items (array of {name: string, brand: string or null, price: number}). " + "If unknown, use null."
          },
          {
            role: "user",
            content: [
    ...
    const total = typeof extracted.total === "number" ? extracted.total : Number(extracted.total) || null;
